...
import {
  tragerCategoriasAPI,
  tragerProductosAPI,
  type CategoriaTrager,
  type ProductoTrager,
  type ProductoTragerPayload,
} from "../../services/api";
import { usePermisos } from "../../hooks/usePermisos";

const EDIT_PRODUCTOS_TRAGER_PERMISSION_MESSAGE =
  "No tienes permiso para editar productos Trager.";
...
const getProductosErrorMessage = (err: unknown, fallback: string): string => {
  ...
  if (apiErr?.response?.status === 403) {
    return EDIT_PRODUCTOS_TRAGER_PERMISSION_MESSAGE;
  }
  return (
...
const calcDisponible = (row: ProductoTrager): number =>
  (row.stock ?? 0) - (row.stockReservado ?? 0);

const isBajoStock = (row: ProductoTrager): boolean => {
...
const ModalProducto: React.FC<{
  ...
  const [categorias, setCategorias] = useState<CategoriaTrager[]>([]);
  ...
  const handleImagenChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!canEditProductos) {
      void message.error(EDIT_PRODUCTOS_TRAGER_PERMISSION_MESSAGE);
      e.target.value = "";
      return;
    }
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      setSaving(true);
      const payload: ProductoTragerPayload = {
      ...
      if (isCreate) {
        await tragerProductosAPI.crear(payload, imagenFile);
        void message.success("Producto creado");
      } else if (isEdit && producto) {
        await tragerProductosAPI.editar(producto.id, payload, imagenFile);
...
const ModalAsignarCategorias: React.FC<{
  ...
  const handleAsignar = async () => {
    ...
    try {
      ...
      const result = await tragerProductosAPI.asignarCategoria(productoIds, categoriaId);
  ...
  const columnasModal: ColumnsType<ProductoTrager> = [
    {
      title: "SKU",
      dataIndex: "sku",
      key: "sku",
...
const TragerProductos: React.FC = () => {
  const { canEdit } = usePermisos();
  const canEditProductos = canEdit("trager_productos");
  ...
  const [seleccionado, setSeleccionado] = useState<ProductoTrager | null>(null);
  ...
  const cargar = useCallback(async () => {
    try {
      ...
      const result = await tragerProductosAPI.listar(params);
      setProductos(result.data);
      setTotal(result.total);
    } catch {
      void message.error("No se pudieron cargar los productos Trager");
      setProductos([]);
    } finally {
      setLoading(false);
    }
  ...
  const abrirEditar = (row: ProductoTrager) => {
    if (!canEditProductos) {
      void message.error(EDIT_PRODUCTOS_TRAGER_PERMISSION_MESSAGE);
      return;
    }
  ...
  const abrirVer = (row: ProductoTrager) => {
    setSeleccionado(row);
    setModalMode("ver");
  };
  ...
  const handleToggleEstado = async (row: ProductoTrager) => {
    if (!canEditProductos) {
      void message.error(EDIT_PRODUCTOS_TRAGER_PERMISSION_MESSAGE);
      return;
    }
    try {
      await tragerProductosAPI.toggleEstado(row.id, !row.activo);
  ...
  const columns: ColumnsType<ProductoTrager> = [
    {
      title: "Imagen",
      key: "imagen",
      width: 70,
...
export default TragerProductos;
