...
import {
  tragerFunnelAPI,
  type TragerDashboardData,
  type TragerDashboardParams,
  type TragerDashboardResponsable,
  type TragerDashboardRiesgoItem,
  type TragerDashboardSinSeguimientoItem,
} from "../../services/api";
...
const TRAGER_PRIMARY = "#e63c1e";
const TRAGER_PRIMARY_DARK = "#c0301a";
...
const PIE_COLORS = [TRAGER_PRIMARY, "#22c55e", "#ef4444", "#f59e0b", "#8b5cf6", "#06b6d4"];
const BAR_CANTIDAD = "#6366f1";
const BAR_MONTO = TRAGER_PRIMARY;
...
const normalizePorEtapa = (
  ...
  const result: Record<string, { cantidad: number; monto: number }> = {};
...
const normalizeRanking = (
  ...
  return raw.map((item) => {
    ...
    return {
      ...
      montoTotal: Number(r.montoTotal ?? r.montoTotal ?? 0),
      montoGanado: Number(r.montoGanado ?? r.montoGanado ?? 0),
    };
  });
};

const normalizeSinSeg = (raw: unknown[]): TragerDashboardSinSeguimientoItem[] => {
  if (!Array.isArray(raw)) return [];
  return raw.map((item) => {
    const r = (item ?? {}) as Record<string, unknown>;
    return {
      ...
      montoEstimado: Number(r.montoEstimado ?? r.monto ?? r.monto ?? 0),
    };
  });
};

const normalizeRiesgoItems = (raw: unknown[]): TragerDashboardRiesgoItem[] => {
  if (!Array.isArray(raw)) return [];
  return raw.map((item) => {
    const r = (item ?? {}) as Record<string, unknown>;
    return {
...
const SeccionProspectos: React.FC<{ d: NonNullable<TragerDashboardData["prospectos"]> }> = ({
  d,
}) => {
...
const SeccionPipeline: React.FC<{
  d: NonNullable<TragerDashboardData["pipelineAvanzado"]>;
}> = ({ d }) => {
  const porResponsable = d?.porResponsable ?? [];
...
const SeccionForecast: React.FC<{ d: NonNullable<TragerDashboardData["forecast"]> }> = ({ d }) => (
  <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
    {(["dias30", "dias60", "dias90"] as const).map((k) => {
      const labels = { dias30: "30 días", dias60: "60 días", dias90: "90 días" };
      const item = d?.[k] ?? { cantidad: 0, monto: 0, montoPonderado: 0 };
      return (
        <div
          key={k}
        ...
        >
          <p
            ...
            style={{ color: TRAGER_PRIMARY_DARK }}
          >
            Forecast {labels[k]}
          </p>
          <p className="text-2xl font-bold" style={{ color: TRAGER_PRIMARY }}>
          ...
          <div className="mt-2 space-y-1 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-mono font-medium">{formatClp(item.monto)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">Monto ponderado</span>
              <span className="font-mono font-medium" style={{ color: TRAGER_PRIMARY }}>
                {formatClp(item.montoPonderado)}
              </span>
            </div>
          </div>
        </div>
...
const SeccionGanadas: React.FC<{ d: NonNullable<TragerDashboardData["ganadas"]> }> = ({ d }) => {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <KpiCard
          ...
          value={formatClp(d?.montoGanadoMesActual)}
          color="#22c55e"
        />
      </div>
      <Card
...
const SeccionMotivos: React.FC<{ d: NonNullable<TragerDashboardData["motivos"]> }> = ({ d }) => (
...
const colsRiesgo = (showUpdatedAt: boolean): ColumnsType<TragerDashboardRiesgoItem> => [
...
const SeccionRiesgo: React.FC<{
  d: NonNullable<TragerDashboardData["riesgoComercial"]>;
}> = ({ d }) => {
    ...
    <div className="space-y-4">
      ...
      >
        {itemsDetenidas.length === 0 ? (
          ...
          <Table<TragerDashboardRiesgoItem>
            rowKey="id"
...
const SeccionConversion: React.FC<{
  d: NonNullable<TragerDashboardData["conversionEtapas"]>;
}> = ({ d }) => {
...
> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo) {
    console.error("[TRAGER_DASHBOARD_ERROR]", error, info);
  }
...
const FunnelTragerDashboard: React.FC<Props> = ({
  ...
  const [rawData, setRawData] = useState<TragerDashboardData | null>(null);
  ...
  const buildParams = useCallback((): TragerDashboardParams => {
    const params: TragerDashboardParams = { diasSinSeguimiento: diasSinSeg };
  ...
  const cargar = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await tragerFunnelAPI.getDashboard(buildParams());
      console.log("[TRAGER_DASHBOARD_DATA]", result);
      setRawData(result);
    } catch (err) {
  ...
  const data = useMemo(() => {
    ...
    return {
      kpis: {
        ...
        pipelineTotal: Number(rawData.kpis?.pipelineTotal ?? 0),
        montoGanado: Number(rawData.kpis?.montoGanado ?? 0),
        montoPerdido: Number(rawData.kpis?.montoPerdido ?? 0),
        tasaCierre: Number(rawData.kpis?.tasaCierre ?? 0),
        tasaRecompraTrager: Number(rawData.kpis?.tasaRecompraTrager ?? 0),
      ...
      sinSeguimiento: {
        total: Number(rawData.sinSeguimiento?.total ?? 0),
        diasSinSeguimiento: Number(rawData.sinSeguimiento?.diasSinSeguimiento ?? diasSinSeg),
        items: normalizeSinSeg(
          rawData.sinSeguimiento?.items ?? []
        ),
      },
      proximasAcciones: {
        vencidas: Number(rawData.proximasAcciones?.vencidas ?? 0),
        hoy: Number(rawData.proximasAcciones?.hoy ?? 0),
        proximos7Dias: Number(rawData.proximasAcciones?.proximos7Dias ?? 0),
      },
  ...
  const columnsResponsables: ColumnsType<TragerDashboardResponsable> = [
  ...
  const columnsSinSeg: ColumnsType<TragerDashboardSinSeguimientoItem> = [
    {
      title: "Oportunidad",
        ...
        >
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Button
              ...
              loading={loading}
              onClick={() => void cargar()}
              style={{ background: TRAGER_PRIMARY, borderColor: TRAGER_PRIMARY }}
            >
              Aplicar filtros
            </Button>
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
              ...
              <KpiCard
                ...
                value={formatClp(data.kpis.pipelineTotal)}
                sub="CLP total activas"
                color={TRAGER_PRIMARY}
              />
              <KpiCard
                label="Monto ganado"
                value={formatClp(data.kpis.montoGanado)}
                color="#22c55e"
              />
              <KpiCard
                label="Monto perdido"
                value={formatClp(data.kpis.montoPerdido)}
                color="#ef4444"
              />
              <KpiCard
              ...
              <KpiCard
                label="Tasa recompra"
                value={`${safeFixed(data.kpis.tasaRecompraTrager)}%`}
                color="#8b5cf6"
              />
              <KpiCard
              ...
              <KpiCard
                ...
                value={formatNum(data.sinSeguimiento.total)}
                sub={`+${diasSinSeg} días sin movimiento`}
                danger={data.sinSeguimiento.total > 0}
              />
              <KpiCard
                label="Acciones vencidas"
                value={formatNum(data.proximasAcciones.vencidas)}
                danger={data.proximasAcciones.vencidas > 0}
              />
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <ProximasCard
                  label="Vencidas"
                  value={data.proximasAcciones.vencidas}
                  color="#dc2626"
                  bg="border border-red-200 bg-red-50"
                />
                <ProximasCard
                  label="Hoy"
                  value={data.proximasAcciones.hoy}
                  color="#d97706"
                  bg="border border-amber-200 bg-amber-50"
                />
                <ProximasCard
                  label="Próximos 7 días"
                  value={data.proximasAcciones.proximos7Dias}
                  color="#2563eb"
            ...
            <div>
              ...
              {data.rankingResponsables.length === 0 ? (
                ...
                <Table<TragerDashboardResponsable>
                  rowKey="responsable"
            ...
            <div>
              <div className="mb-3 flex flex-wrap items-center gap-2">
                ...
                {data.sinSeguimiento.total > 0 && (
                  <Tag color="orange">
                    {data.sinSeguimiento.total} oportunidad
                    {data.sinSeguimiento.total !== 1 ? "es" : ""} sin movimiento
                    en más de {data.sinSeguimiento.diasSinSeguimiento} días
                  </Tag>
                )}
              </div>
              {data.sinSeguimiento.items.length === 0 ? (
                ...
                <Table<TragerDashboardSinSeguimientoItem>
                  rowKey="id"
                  columns={columnsSinSeg}
                  dataSource={data.sinSeguimiento.items}
                  size="small"
                  pagination={{ pageSize: 10, showSizeChanger: false }}
                  scroll={{ x: 900 }}
...
export default FunnelTragerDashboard;
