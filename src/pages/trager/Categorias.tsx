...
const EDIT_CATEGORIAS_TRAGER_PERMISSION_MESSAGE =
  "No tienes permiso para editar categorías Trager.";

type CategoriaFormValues = {
  nombre: string;
};

type CategoriaRow = CategoriaTrager & {
...
const getCategoriasErrorMessage = (error: unknown, fallback: string): string => {
  ...
  if (apiError?.response?.status === 403) {
    return EDIT_CATEGORIAS_TRAGER_PERMISSION_MESSAGE;
  }
...
const TragerCategorias = () => {
  const { canEdit } = usePermisos();
  const canEditCategorias = canEdit("trager_categorias");
  ...
  const cargarCategorias = async () => {
    try {
      setLoading(true);
      const data = await tragerCategoriasAPI.listar();
      setCategorias(data as CategoriaRow[]);
    } catch (error) {
      console.error("Error al cargar categorias Trager:", error);
  ...
  const abrirCrear = () => {
    if (!canEditCategorias) {
      message.error(EDIT_CATEGORIAS_TRAGER_PERMISSION_MESSAGE);
      return;
    }
  ...
  const guardarCategoria = async () => {
    ...
    try {
      ...
      if (categoriaEditando) {
        await tragerCategoriasAPI.editar(categoriaEditando.id, nombre);
        message.success("Categoria actualizada correctamente");
      } else {
        await tragerCategoriasAPI.crear(nombre);
      ...
      console.error("Error al guardar categoria Trager:", error);
  ...
  const eliminarCategoria = async (categoria: CategoriaRow) => {
    ...
    try {
      await tragerCategoriasAPI.eliminar(categoria.id);
      message.success("Categoria eliminada correctamente");
      await cargarCategorias();
    } catch (error) {
      console.error("Error al eliminar categoria Trager:", error);
...
export default TragerCategorias;
