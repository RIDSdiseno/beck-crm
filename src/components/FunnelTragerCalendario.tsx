...
import type { TragerFunnelEtapa, TragerFunnelOportunidad } from "../services/api";

type CalendarMode = "proximaAccion" | "fechaProbableCierre" | "fechaCierre";

type FunnelTragerCalendarioProps = {
  oportunidades: TragerFunnelOportunidad[];
  onOpenDetail: (oportunidad: TragerFunnelOportunidad) => Promise<void> | void;
};

type OportunidadConFechaCierre = TragerFunnelOportunidad & {
  fechaCierre?: string | null;
};

const ETAPA_COLORS: Record<TragerFunnelEtapa, string> = {
  PROSPECTO: "#6b7280",
...
const ETAPA_LABELS: Record<TragerFunnelEtapa, string> = {
  PROSPECTO: "Prospecto",
...
const proximaAccionColor = (dateStr: string): string => {
  ...
  if (fecha.getTime() === today.getTime()) return TRAGER_COLOR;
  return "#2563eb";
};

export default function FunnelTragerCalendario({
  oportunidades,
  onOpenDetail,
}: FunnelTragerCalendarioProps) {
  ...
  const events = (() => {
    ...
    return oportunidades
      ...
      .map((o) => {
        const color = ETAPA_COLORS[o.etapa] ?? TRAGER_COLOR;
        return {
          id: String(o.id),
          title: o.nombreOportunidad ?? o.cliente,
      ...
      <FullCalendar
        ...
        eventClick={(info) => {
          const oportunidad = info.event.extendedProps
            .oportunidad as TragerFunnelOportunidad;
